...
class TransactionProcessorService {
  ...
  start (callback) {
    this._mongo.start((err) => {
      ...
      this._queue.process('addresses.connectTransactions', (job, done) => {
        ...
        try {
          this._loadTransactions(job.data.addresses, job.data.userId, (err, transactions) => {
            ...
            return transactionProcessor.process(transactions, (err) => {
              if (err) {
                log.error(err)
                return done(err)
              }
              this._jobs.addJob('addresses.update', { addresses: job.data.addresses, userId: job.data.userId }, done)
            })
          })
        } catch (err) {
          log.error(err)
