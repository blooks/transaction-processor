  ...
  _saveWallets (callback) {
    ...
    async.forEach(this.affectedWalletObjects,
      (wallet, callback) => {
        ...
        if (this._jobs) {
          this.affectedWalletObjects.forEach((wallet) => {
            this._jobs.wallet.update({ walletId: wallet._id, userId: wallet.userId })
          })
        } else {
      ...
      this._updateInOutputs(transaction, (err, details) => {
        ...
        if (this._converter) {
          return async.mapSeries([ 'EUR', 'USD' ], (currency, callback) => {
            this._converter.convert('BTC', currency, amount, date, (err, resp) => {
              var exchangeRate = {}
